...
import { render, cleanup, getByText, queryByText, fireEvent } from "@testing-library/react";
...
describe("Form", () => {
  ...
  it("validates that the interviewer cannot be null", () => {
    ...
    const onSave = jest.fn();
    const { getByText } = render(<Form interviewers={interviewers} onSave={onSave} student={"test"} interviewer={null} />);
    fireEvent.click(getByText("Save"));
  ...
  it("calls onSave function when the name and interviewer is defined", () => {
    ...
    const onSave = jest.fn();
    const { queryByText } = render(<Form interviewers={interviewers} student={"Lydia Miller-Jones"} interviewer={1} onSave={onSave} />)
